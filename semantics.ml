...
let rec analyze_expr expr env =
  ...
  | Syntax.Call c -> 
     match Env.find_opt c.func env with
      | Some (Func_t (rt, at)) ->  if List.length at != List.length c.args then
                                      ...
                                      let args = List.map2 (fun eat a -> let aa, at = analyze_expr a env
                                                  ...
                                                  at c.args in  Call (c.func, args), rt    
...
let rec analyze_instr instr env =
  ...
  | Syntax.Assign a ->  if Env.mem a.var env then
                            ...
                            match et with 
                              ...
                              | Bool_t(Bool_t,true) -> let vt = Env.find a.var env in
                                                       if verify_type vt et then
  ...
  | Syntax.Return r -> let ae, _ = analyze_expr r.expr env in
                       Return ae, env
  ...
  |Syntax.Boucle_while w ->  let ana_cond, type_cond = analyze_expr w.condit  env  in
                             let ana_blco_w, new_env = analyze_block w.bloc_w env  in
                             Boucle_while(ana_cond, ana_blco_w), new_env

                    
  |Syntax.Call_func f ->  let ana_call, type_Call_fun = analyze_expr f.appel env in
